...
4. 配置：`pipeline.yaml` 的 `stage_providers` 增加该阶段（默认 `codex`）。
...
- **读反馈**：用 T03 的 `SupabaseStore.fetch_undigested_feedback(section_slug)`；按 `issue_date` 分组，每组用本地那期日报 `web/runs/issues/<issue_id>.json`（`load_issue`）把反馈对回具体条目（`article_level`+`article_index` / `source_item_ids`）。
