...
标题铁律：
1. 标题必须不超过 20 个字符；20 是硬上限，超过会被本地拒绝并阻断整次导出。程序按 Python `len()` 计数，英文字母、空格和标点都各算 1 个字符；输出前必须自查。
2. 忠实、不标题党、不新增事实；只能使用输入文件 `headlines[0]` 里的标题、摘要、AI 分析和来源信息。该条已是小红书选中的封面新闻，不得另选其他新闻角度。
3. 不得改写关键数字、主体公司/人物、时间、地点；不确定就写更概括的标题。
4. 尚未生效状态、未来生效时间和关键触发条件也是关键事实。原文含“最早/最快/不早于”等限定时，必须保留限定词和对应日期；“可”不能单独替代未来状态。
5. 标题空间不足时，按“主体 → 状态/生效时间 → 触发条件 → 核心动作 → 其他数字”保留信息。
6. 不要概括整期，不要写刊名、日期或“今日看点”；只从 `headlines[0]` 中挑一个最有信息密度的具体角度。
7. 标题必须包含至少一个来自输入的具体信号：主体公司/人物、关键数字、产品/技术名、交易动作或趋势词。
8. 标题应该像一个钩子，但只能突出当天最值得看的真实角度，不要夸张、煽动或制造未发生的因果。
9. 不要使用省略号（… 或 ...）、换行、引号、Markdown。
...
- 科技新闻来了
- 一文看懂AI
- 任何只表达“日报 / 看点 / 速览 / 简报 / 科技资讯”的标题

推荐标题方向：
- 抓“反常识”：某个数字、利润、涨价、部署比例、融资/IPO/ADR 动作背后的变化。
- 抓“趋势转折”：例如一个行业周期、一个生产环境信号、一个基础设施权力变化。
- 抓“具体主体”：优先用输入里的公司、人物、产品、技术名，而不是泛泛写 AI 或科技。
- 状态事实示例：“美国最大电网将在缺电时限供大型数据中心，最早2027年6月启动”可写成“最早2027年6月PJM缺电时可限供”，不能写成“PJM缺电时限供50兆瓦以上数据中心”。

输入文件字段：
- publication_name / issue_date / date_cn：本期元数据。
- title_max_chars：标题硬上限。
- headlines：只含一条，即已选中的小红书封面新闻。
...
def build_xhs_note_title_repair_prompt(
    input_path: Path,
    failed_output: XHSNoteTitleOutput,
    validation_errors: list[str],
) -> str:
    return f"""
{build_xhs_note_title_prompt(input_path)}

这是唯一一次语义修正。上一次输出通过了 JSON 结构校验，但未通过发布标题事实保护：
{json.dumps(failed_output.model_dump(mode="json"), ensure_ascii=False, indent=2)}

程序给出的具体错误：
{json.dumps(validation_errors, ensure_ascii=False, indent=2)}

请只做解决错误所需的最小修改，继续只使用 headlines[0]。若涉及未来状态、生效时间或触发条件，必须保留原文限定；不要只补“可”。修正后仍须不超过 20 个字符。只输出 JSON 对象，不要解释。
""".strip()
...
3. 尚未生效状态、未来生效时间及关键触发条件也是受保护事实。原文含“最早/最快/不早于”等时间限定时，标题必须保留限定词和对应日期；“可”只表示能力或权限，不能单独替代未来状态。
4. 标题空间不足时，按“主体 → 状态/生效时间 → 触发条件 → 核心动作 → 其他数字”保留信息。宁可把次要数字放到支撑句，也不能把未来政策写得像当前已经生效。
5. 只允许调整表述：冲击数/反常点前置、去通稿腔、突出原文已有的反差/痛点、留不欺骗的信息缺口。
6. 两条都必须保留输入中的真实主体，禁止加入输入不存在的公司、人物或对比对象。

两档要求：
- restrained：克制版，默认上封面；12-24 字，专业、抓眼，不用震惊、太火了、暴涨、崩盘、吊打、秒杀、逆天等情绪词。
- punchy：冲版，只存作备选；12-24 字，可有限口语化，但事实层同样锁死。
- 12-24 指程序按 Python `len()` 计算的字符数；英文字母、空格和标点都各算 1 个字符。两条输出前必须自查字符数，英文主体过长时应删去次要修饰词，不得截断英文单词。
...
- 美国最大电网将在缺电时限供大型数据中心，最早2027年6月启动
  → 最早2027年6月起，PJM缺电时可限供数据中心 ｜ 最早2027年6月，PJM缺电时或限供数据中心

反例：
- 智谱估值暴涨15倍：ARR 被偷换成估值。
- 震惊！智谱ARR暴涨：震惊体且程度夸大。
- 谷歌芯片吊打英伟达：无中生有的对比和主体。
- PJM缺电时可限供50兆瓦及以上数据中心：遗漏“最早2027年6月启动”，容易被理解为已经生效。
...
def build_xhs_magnetize_repair_prompt(
    input_path: Path,
    failed_output: XHSMagnetizeOutput,
    validation_errors: list[str],
) -> str:
    failed_payload = failed_output.model_dump(mode="json")
    return f"""
{build_xhs_magnetize_prompt(input_path)}

这是唯一一次语义修正。上一次输出通过了 JSON 结构校验，但克制版未通过事实保护：
...
修正规则：
1. 只做解决上述错误所需的最小修改，不重新选择新闻角度。
2. 若错误涉及未来状态或生效时间，必须从输入原文逐字保留日期及“最早/最快/不早于”等限定；不要只补一个“可”字。
3. 若错误涉及触发条件，必须保留原文条件，不得把有条件动作写成无条件事实。
4. 修正后的 restrained 和 punchy 都仍须满足 12-24 字、真实主体、原文数字和克制程度要求。
5. 只输出 JSON 对象，不要解释。
""".strip()
