...
4. 不要概括整期，不要写刊名、日期或“今日看点”；只从 `headlines[0]` 中挑一个最有信息密度的具体角度。
5. 标题必须包含至少一个来自输入的具体信号：主体公司/人物、关键数字、产品/技术名、交易动作或趋势词。
6. 标题应该像一个钩子，但只能突出当天最值得看的真实角度，不要夸张、煽动或制造未发生的因果。
7. 不要使用省略号（… 或 ...）、换行、引号、Markdown。
